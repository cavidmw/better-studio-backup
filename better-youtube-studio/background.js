    ...
    chrome.storage.local.get(['collections_data'], (result) => {
      if (!result.collections_data) {
        chrome.storage.local.set({
          ...
          console.log('BYS Collections: Default storage initialized.');
        });
      }
    });
  }
  console.log('BYS Background Service Worker installed/updated:', details.reason);
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'openTabs') {
    ...
    const openPromises = urls.map(url => {
      return chrome.tabs.create({ url, active: false }).catch(err => {
        console.warn('Tab could not be opened:', url, err);
        return null;
      });
    });
