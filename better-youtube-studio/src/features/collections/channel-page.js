...
window.BYS.Collections.ChannelPage = (() => {
  ...
  const handleButtonClick = (btn) => {
    ...
    if (!channelInfo || !channelInfo.id) {
      console.error('BYS Collections: Could not extract channel info on click');
      return;
    }
  ...
  const forceReset = () => {
    ...
    resetButtonToDefault(btn);
    console.log('BYS Collections: State force-reset ✓');
  };
  ...
  const inject = async () => {
    ...
    try {
      ...
      if (!synced) { console.log('BYS Collections: DOM sync failed or was aborted'); return false; }
      ...
      try {
        ...
        if (err.name === 'AbortError') { console.log('BYS Collections: inject aborted (navigation)'); return false; }
        console.log('BYS Collections: target not found', err.message);
        return false;
      }
      ...
      console.error('BYS Collections: Error injecting button', err);
      return false;
    } finally {
      isInjecting = false;
    }
