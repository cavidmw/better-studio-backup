// BYS Collections - Masthead Button Component
// Namespace: window.BYS.Collections.MastheadButton

window.BYS = window.BYS || {};
window.BYS.Collections = window.BYS.Collections || {};

window.BYS.Collections.MastheadButton = (() => {
  // ─── i18n Helper ────────────────────────────────────────────────────────
  function t(key, params = {}) {
    return window.BYS?.i18n?.t?.(key, params) || key;
  }

  // 3D Folder icon — kırmızı tema (mfg ID prefix'leri korunuyor)
  ...
  const createMastheadButton = () => {
    ...
    btn.innerHTML = `${folder3DIcon}<span class="ytc-masthead-btn-text">${t('collections.masthead.btn')}</span>`;
  ...
  const inject = () => {
    if (document.getElementById('ytc-collections-masthead-entry')) return true;
  ...
  const exists = () => !!document.getElementById('ytc-collections-masthead-entry');
