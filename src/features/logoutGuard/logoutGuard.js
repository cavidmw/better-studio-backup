...
window.BYS.LogoutGuard = (function () {
    ...
    function isLogoutElement(element) {
        ...
        const container = element.closest('tp-yt-paper-item, ytd-compact-link-renderer, ytd-menu-service-item-renderer, a.yt-simple-endpoint, a[href*="logout"]');
        if (!container) return false;

        // Bypass kontrolü container'da da
        if (container.dataset?.bysAllowed === 'true') return false;

        // href tabanlı kesin tespit (YouTube genellikle çıkış linklerinde /logout kullanır)
        const href = container.href || container.getAttribute('href') || '';
        if (href.toLowerCase().includes('logout')) return container;

        // Container içinde logout linki varsa
        const innerLink = container.querySelector('a[href*="logout"]');
        if (innerLink) return container;

        const lowerLogoutTexts = LOGOUT_TEXTS.map(t => t.toLowerCase());

        // Label metni kontrolü
        const label = container.querySelector('#label, yt-formatted-string, [id="label"]');
        if (label) {
            const text = label.textContent?.trim().toLowerCase();
            if (lowerLogoutTexts.some(t => text?.includes(t))) {
                return container;
            }
        }
