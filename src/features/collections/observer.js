...
window.BYS.Collections.Observer = (() => {
  ...
  const handlers = {
    ytNavigateStart: () => abortNavigation(),
    ytNavigateFinish: () => fireNavigate(),
    ytPageDataUpdated: () => firePageDataUpdated(),
    popstate: () => setTimeout(fireNavigate, 0)
  };
  ...
  const bindYTEvents = () => {
    ...
    eventsBound = true;
    document.addEventListener('yt-navigate-start', handlers.ytNavigateStart);
    document.addEventListener('yt-navigate-finish', handlers.ytNavigateFinish);
    document.addEventListener('yt-page-data-updated', handlers.ytPageDataUpdated);
    window.addEventListener('popstate', handlers.popstate);
  };

  const unbindYTEvents = () => {
    if (!eventsBound) return;
    eventsBound = false;
    document.removeEventListener('yt-navigate-start', handlers.ytNavigateStart);
    document.removeEventListener('yt-navigate-finish', handlers.ytNavigateFinish);
    document.removeEventListener('yt-page-data-updated', handlers.ytPageDataUpdated);
